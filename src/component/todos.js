      ...
      todos.map((todo) => {
        return (
          <Grid key={todo.id} className="grid">
            <Card className="card">
              <CardContent className="cardContent" key={todo.id}>
                <span className="todoContent">{todo.content}</span>
                <button className="button" onClick={() => removeTodo(todo.id)}>Remove Task</button>
              </CardContent>
            </Card>
          </Grid>
        );
